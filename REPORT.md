...
## Phase 3 Task 2 (attempt 1): Pretrained ChemBERTa features do not help
Task 1 showed that hand-crafted descriptors plus XGBoost fail on the S/P-rich electrolyte molecules, with exotic-element R2 near zero or negative. Following the Mena review Section 8 idea of unsupervised pretraining plus fine-tuning, we tested whether pretrained ChemBERTa (`DeepChem/ChemBERTa-77M-MLM`, pretrained on 77M PubChem SMILES) gives a representation that handles S/P chemistry better. Unlike the QM9 SchNet route, ChemBERTa works from SMILES and handles all elements in this dataset, so all 3,663 molecules were used. We extracted 384-dimensional mean-pooled and CLS-token features, then compared A = RDKit concat, B = ChemBERTa, and C = RDKit + ChemBERTa on the identical mentor split: train on 2,140 baseline molecules and test on 1,523 agent molecules.

The result is plainly negative. For the sulfur-containing exotic test subset, ESP R2 was A = -0.24, B1(mean) = -0.50, B2(CLS) = -0.75, C_mean = -0.26, and C_cls = -0.35. For Zn binding, sulfur-subset R2 was A = -0.05, B1 = -0.12, B2 = -0.11, C_mean = -0.13, and C_cls = -0.07. Every ChemBERTa-only variant and every RDKit+ChemBERTa concatenation was equal to or worse than the RDKit baseline for both targets and across the element groups. Even on QM9-compatible molecules, ChemBERTa was far below RDKit for ESP: R2 = 0.22 for mean-pooled ChemBERTa versus 0.77 for RDKit concat.

The interpretation is that ChemBERTa's masked-language pretraining on drug-like SMILES is weakly related to these quantum-chemical targets: ESP minimum and Zn binding for small electrolyte additives. Recognizing the symbol `S` in a SMILES string is not the same as representing sulfur's physical contribution to binding energy, which is consistent with the review's warning about negative transfer from a mismatched pretraining task or domain.

The result is still trustworthy and useful. A-only exactly reproduced Task 1, and the row-by-row `canonical_smiles` alignment check between RDKit features, ChemBERTa features, and labels passed before any concatenation. This eliminates the cheap "reuse a pretrained language model" route as a fix for the S/P gap; the more promising but heavier direction is a representation trained on 3D structure plus quantum-chemical data that actually covers S/P chemistry.
